...
#	rMin = 1.
#	a = (8.)*math.pi*(1/T)*((-1/(3*pow(rc,3)))-(-1/(3*pow(rMin,3))))
	return a
...
#	return (-1+(1+2*Ma*rho*D)**0.5)/(Ma*rho*D)
...
#	return -(2*Ma)/(math.sqrt(1+4*Ma*rho*D)*pow(1+math.sqrt(1+4*Ma*rho*D),2.)) * (D + rho*Dp) 
	...
	#return (1/(rho*D*((1+2*Ma*rho*D)**0.5)))*(D+rho*Dp) + ((1-(1+2*Ma*rho*D)**0.5)/(Ma*rho*rho*D*D))*(D+rho*Dp)
...
T_vec = np.array([0.7, 0.8, 0.9, 1.0])
...
for i in range (len(T_vec)):
	...
	d = 0.029112
#	print fmu_id(d,M) + fmu_hs(d) + fmu_disp(d,a)
#	print fp_id(d,M) + fp_hs(d) + fp_disp(d,a)
#	exit(1)

	# normalizeation T_assoc relative to the T
	#T_assoc_norm = T_assoc*T
	...
	oMat.append([T, mu_coex, rho_v, rho_l, (pv+pl)*0.5, (pv+pl)*0.5])
	o2Mat.append([T, mu_coex])
	o3Mat.append([T, m])
	o4Mat.append([T, a])

np.savetxt(o1filename,oMat,fmt='%5.10f')
np.savetxt(o2filename,o2Mat,fmt='%5.10f')
np.savetxt(o3filename,o3Mat,fmt='%5.10f')
np.savetxt(o4filename,o4Mat,fmt='%5.10f')
